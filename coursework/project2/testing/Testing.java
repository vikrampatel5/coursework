...
public class Testing {
    ...
    public static Function<Boolean, List<Message>> test_12 = (Boolean isGlobalLock) -> {
        ...
        var r = new RainforestShop(ofUserNames, Collections.emptyMap(), isGlobalLock);
        ...
        if(s.isStopped())
             first_test.add(new Message(true,"GOOD: The supplier is stopped by receiving a @stop! message only when the stopSupplier method is invoked"));
        else first_test.add(new Message(false,"ERROR: The supplier is stopped by receiving a @stop! message only when the stopSupplier method is invoked"));
    ...
    public static Function<Boolean,List<Message>> test_03 = (Boolean isGlobalLock) -> {
        ...
        /// TEST 1) Cannot basket unavailable product
        {
            ...
            var s1 = new RainforestShop(ofUserNames, Collections.emptyMap(), isGlobalLock);
            var s2 = new RainforestShop(ofUserNames, Collections.emptyMap(), isGlobalLock);

            //Created two different threads for same user to try login 2 times.
            var pool = Executors.newFixedThreadPool(2);
            pool.execute(s1);
            pool.execute(s2);
        ...
        ofUserNames.add("bogus");
        {
            ...
            try {
                ...
                result1.ifPresent(transaction -> {
                    ...
                    if (transaction.shelfProduct(new Item("product", 1.0, BigInteger.valueOf(14)))) {
